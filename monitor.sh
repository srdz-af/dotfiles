...
Lan(){
	LANINFO=$(cat /proc/net/wireless | awk 'FNR == 3 {print $4}' | tr -d .-)
...
Bat(){	
	...
	BATINFO=$(upower -i /org/freedesktop/UPower/devices/battery_BAT0 | grep percentage | awk '{print $2}')
	BATNUM=${BATINFO::-1}

	if [[ $(upower -i /org/freedesktop/UPower/devices/battery_BAT0 | grep state | awk '{print $2}') == "charging" ]]
	then 
		BATICON=""
	else 
